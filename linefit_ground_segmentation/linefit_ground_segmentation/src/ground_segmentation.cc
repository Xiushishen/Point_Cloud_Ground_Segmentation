...
void GroundSegmentation::segment(const PointCloud& cloud, vector<int>* segmentation) {
  cout << "Segmenting cloud from Livox HAP Lidar with " << cloud.size() << " points." << endl;
  ...
  resetSegments();
  // get the index of bin and segment for every point cloud
  insertPoints(cloud);
  list<PointLine> lines;
  ...
  cout << "Segmentation Successfully " << fp_ms.count() << "ms\n"; 
}

void GroundSegmentation::getLines(list<PointLine>* lines) {
  mutex line_mutex;
  vector<thread> thread_vec(params_.n_threads);
  for (uint i = 0; i < params_.n_threads; ++i) {
    const uint start_index = params_.n_segments / params_.n_threads * i;
    const uint end_index = params_.n_segments / params_.n_threads * (i + 1);
    thread_vec[i] = thread(&GroundSegmentation::lineFitThread, this,
...
void GroundSegmentation::lineFitThread(const uint start_index,
  ...
  for (uint i = start_index; i < end_index; ++i) {
    ...
    if (visualize) {
      list<Segment::Line> segment_lines;
...
void GroundSegmentation::resetSegments() {
  segments_ = vector<Segment>(params_.n_segments, Segment(params_.n_bins,
                                                          params_.min_slope,
                                                          params_.max_slope,
...
void GroundSegmentation::assignCluster(vector<int>* segmentation) {
  vector<thread> thread_vec(params_.n_threads);
  const size_t cloud_size = segmentation->size();
  for (uint i = 0; i < params_.n_threads; ++i) {
    const uint start_index = cloud_size / params_.n_threads * i;
    const uint end_index = cloud_size / params_.n_threads * (i + 1);
    thread_vec[i] = thread(&GroundSegmentation::assignClusterThread, this,
...
void GroundSegmentation::assignClusterThread(const uint& start_index,
                                             const uint& end_index,
  ...
  for (uint i = start_index; i < end_index; ++i) {
    ...
    if (segment_index >= 0) {
      ...
      while (dist < 0 && steps * segment_step < params_.line_search_angle) {
        ...
        // index_1 = (segment_index + steps) % params_.n_segments;
        // index_2 = (segment_index - steps + params_.n_segments) % params_.n_segments;
        ...
        // dist = max(dist, max(dist_1, dist_2));
        ++steps;
      }
...
void GroundSegmentation::insertPoints(const PointCloud& cloud) {
  ...
  threads[params_.n_threads - 1] = 
      thread(&GroundSegmentation::insertionThread, this, cloud, start_index, end_index);
...
void GroundSegmentation::insertionThread(const PointCloud& cloud,
  ...
  const double segment_step = 2 * M_PI / params_.n_segments;
  const double bin_step = (sqrt(params_.r_max_square)
                           - sqrt(params_.r_min_square)) / params_.n_bins;
  const double r_min = sqrt(params_.r_min_square);
  for (uint i = start_index; i < end_index; ++i) {
    ...
    if (range_square < params_.r_max_square && range_square > params_.r_min_square) {
      ...
      const uint bin_index = (range - r_min) / bin_step;
      const uint segment_index = (angle + M_PI) / segment_step;
      const uint segment_index_clamped = segment_index == params_.n_segments ? 0 : segment_index;
      segments_ [segment_index_clamped][bin_index].addPoint(range, point.z);
      bin_index_[i] = make_pair(segment_index_clamped, bin_index);
    } else {
        bin_index_[i] = make_pair<int, int>(-1, -1);
    }
